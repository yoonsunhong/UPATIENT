...
function drawChart1() {
	 jQuery.ajax({
		...
		var $bgFigure = $('.bg_info .day_info .f_l:first-child').innerWidth();
		var $bgState = $('.bg_info .day_info .f_l:nth-child(2)').innerWidth();
		var $bgPad = parseInt($('.bg_info .day_info .clear').css('paddingLeft'))*2;
	
		$('.bg_info .day_info .clear').width($bgState+$bgFigure+$bgPad+1);	
