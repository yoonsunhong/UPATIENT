	  	...
	  	jQuery.ajax({
		    ...
		    success : function(data) {
	    		...
	    		$.each(data.rows, function(key, value) {   
					...
					if(this.cell[11]=="01"){
						bmiState = "low";
				 	}else if(this.cell[11]=="02"){
				 		bmiState = "normal";
				 	}else if(this.cell[11]=="03"){
				 		bmiState = "warning";
				 	}else if(this.cell[11]=="04"){
				 		bmiState = "high";
				 	}else if(this.cell[11]=="05"){
				 		bmiState = "danger";
				 	}
					
		    	     	
	   	     		//divHtmlLi = "<li><div class=\"area\"><a href=\"#\" onClick=\"bodyMassIndexMstView('" + this.cell[0] + "','" + this.cell[1] + "','" + this.cell[2] + "','" + this.cell[3] + "','" + this.cell[4] + "','" + this.cell[5] + "','" + this.cell[6] + "','" + this.cell[7] + "')\"><p class=\""+sysState+"\"><strong class=\"figure\">" + this.cell[3] + "</strong><strong><span><spring:message code='systole'/></span><span>mmHg</span></strong></p><p class=\""+diasState+"\"><strong class=\"figure\">" + this.cell[4] + "</strong><strong><span><spring:message code='diastole'/></span><span>mmHg</span></strong></p><p><strong>" + this.cell[5] + "</strong><strong><span><spring:message code='heartRate'/></span><span>Beats/Min</span></strong></p><p class=\"alarm\"><strong>" + this.cell[2] + "</strong></p></a></div></li>";
	   	     		divHtmlLi = "<li><div class=\"area\"><a href=\"#\" onClick=\"bodyMassIndexMstView('" + this.cell[0] + "','" + this.cell[1] + "','" + this.cell[2] + "','" + this.cell[3] + "','" + this.cell[4] + "','" + this.cell[5] + "','" + this.cell[6] + "','" + this.cell[7] + "','" + this.cell[8] + "','" + this.cell[9] + "','" + this.cell[10] + "','" + this.cell[11] + "')\"><p class=\""+bmiState+"\"><span></span><strong class=\"figure\"></strong></p><p><strong>" + this.cell[3] + "</strong><strong><span><spring:message code='weight'/></span><span>kg</span></strong></p><p><strong>" + this.cell[6] + "</strong><strong><span><spring:message code='fatRate'/></span><span>%</span></strong></p><p><strong>" + this.cell[7] + "</strong><strong><span>BMI</span><span>kg/m2</span></strong></p><p class=\"alarm\"><strong>" + this.cell[2] + "</strong></p></div></li>";	
...
//			          alert("에러발생111");
	...
	//혈압측정 수정페이지 이동
	function bodyMassIndexMstView(pMeasureSeq, pMeasureDt, pMeasureTime, pWeight, pMuscleQy, pFatQy, pFatRate, pBmi, pBoneQy, pMoistQy, pVisFatLvl, pBmiState){
		...
		var weight = pWeight;
		var muscleQy = pMuscleQy;
		var fatQy = pFatQy;
		var fatRate = pFatRate;
		var bmi = pBmi ;
		var boneQy = pBoneQy;
		var moistQy = pMoistQy;
		var visFatLvl = pVisFatLvl;
		var bmiState = pBmiState;
		...
		$('#WEIGHT').val(weight);
		$('#MUSCLE_QY').val(muscleQy);
		$('#FAT_QY').val(fatQy);
		$('#FAT_RATE').val(fatRate);
		$('#BMI').val(bmi);
		$('#BONE_QY').val(boneQy);
		$('#MOIST_QY').val(moistQy);
		$('#VIS_FAT_LVL').val(visFatLvl);
		$('#BMI_STATE').val(bmiState);
		
		var frm = document.searchForm;
		frm.action = "/bodyMassIndexMstView.do?lang="+lang;
		frm.submit();
		...
		<header id="header">
			<div class="area">
				<div>
					...
					<h1 class="tit"><spring:message code="bmiReport"/><!-- BMI리포트 --></h1>
				...
				<div class="tab_cnt">
					<div>
						<form action="" id="searchForm" name="searchForm" method="post">
							...
							<input type="hidden" id="MUSCLE_QY" name="MUSCLE_QY" VALUE="">
							<input type="hidden" id="FAT_QY" name="FAT_QY" VALUE="">
							<input type="hidden" id="FAT_RATE" name="FAT_RATE" VALUE="">
							<input type="hidden" id="BMI" name="BMI" VALUE="">
							<input type="hidden" id="BONE_QY" name="BONE_QY" VALUE="">
							<input type="hidden" id="MOIST_QY" name="MOIST_QY" VALUE="">
							<input type="hidden" id="VIS_FAT_LVL" name="VIS_FAT_LVL" VALUE="">
